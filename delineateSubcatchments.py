# Delineating subcatchemnts around inundation hazardspots
...
for prefix in ("10a", "30a", "50a", "100a"):
    ...
    for i in range(1, 16):
        ...
        hazardspots = wd > th
        hazardspots = hazardspots + permanent_water # add permanent water to hazardspots
        hazardspots[hazardspots > 1] = 1 # in case of overlap, make sure that result is binary
        hazardspots_pcr = numpy2pcr(Boolean, hazardspots, 0)
        hazardspot_ids = clump(hazardspots_pcr)
        hazardspot_ids_np = pcr2numpy(hazardspot_ids, 0)
 
        if min_size > 0:
            ids, freq = np.unique(hazardspot_ids_np, return_counts=True)
            filter_ids = ids[freq < min_size]
            hazardspot_ids_np[np.isin(hazardspot_ids_np, filter_ids)] = 0
            hazardspot_ids = numpy2pcr(Nominal, hazardspot_ids_np, 0)
        
        subdelineation = pcraster.subcatchment(flowdir, hazardspot_ids)
